...
## Corpus Check
- 25 files · ~67,929 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 151 nodes · 272 edges · 16 communities (9 shown, 7 thin omitted)
...
5. `TicketRow` - 10 edges
6. `EmailService` - 10 edges
7. `ApiSource` - 9 edges
...
- `Ticket Build Form` --references--> `admin_create_ticket()`  [EXTRACTED]
  templates/dashboard.html → web_app.py
- `ApiDownloader` --uses--> `ApiSource`  [INFERRED]
  api_downloader.py → api_registry.py
...
Cohesion: 0.10
Nodes (7): Database, TicketRow, datetime, 24-Hour File Message Deletion, Ticket Redeem Flow, TicketCreateResult, TicketService
...
### Community 2 - "builder.py"
Cohesion: 0.25
Nodes (6): ApiRegistry, ApiSource, Path, BuildResult, API Selection Modes, Telegram GameHub Bot VPS Mode

### Community 3 - "EmailService"
Cohesion: 0.23
Nodes (9): _choose_addgame_file(), _choose_bypass_template(), ensure_directories(), load_settings(), _parse_admin_ids(), Path, Settings, EmailService (+1 more)
...
### Community 14 - "test_rebranding.py"
Cohesion: 0.15
Nodes (5): FakeDownloader, FakeRegistry, FakeSMTP, PackageRebrandingTests, Path

### Community 15 - "ApiDownloader"
Cohesion: 0.31
Nodes (4): ApiDownloader, DownloadError, Exception, Path
...
- **Why does `PackageBuilder` connect `PackageBuilder` to `Database`, `web_app.py`, `builder.py`, `EmailService`, `test_rebranding.py`, `ApiDownloader`?**
  _High betweenness centrality (0.201) - this node is a cross-community bridge._
- **Why does `TicketService` connect `Database` to `web_app.py`, `builder.py`, `PackageBuilder`?**
  _High betweenness centrality (0.134) - this node is a cross-community bridge._
- **Why does `Database` connect `Database` to `web_app.py`, `builder.py`?**
...
- **What connects `setup_vps_ubuntu.sh script`, `24-Hour File Message Deletion`, `30-Day Admin Session` to the rest of the system?**
